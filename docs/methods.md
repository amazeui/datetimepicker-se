---
title: Amaze UI DatetimePicker SE 方法列表
next: events.html
prev: options.html
---

# DatetimePicker SE 方法
---
...
<div class="am-alert am-alert-warning">
    <strong>注意：</strong>
    所有方法都通过 <code>data</code> 属性访问，例如
    <pre>
$('#datetimepicker').data('DateTimePicker').someMethod();</pre>
</div>
...
返回选择器当前选中的日期，`moment` 对象，如果没有设置则返回 `null`。

### date([newDate])

设置选择器日期，参数可以为 `string, Date, moment, null`：

- 如果是字符串则使用 `moment` 库根据 `options.format` 和 `options.useStrict` 解析；
- 如果为 `null` 则清空选择器原来选择的日期。
